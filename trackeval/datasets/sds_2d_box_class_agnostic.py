...
class SDS2DBoxClassAgnostic(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': None,  # Valid categories
            'BENCHMARK': 'SDS',  # Valid: 'SMDVIS', 'SMDNIR', 'SMDVISNIR'
            ...
            'IMG_LOC_FORMAT': '{gt_folder}/{seq}/img1',  # '{gt_folder}/{seq}/img1/
            'IMG_EXTENSION': ['*.jpg', '*.jpeg', '*.png', '*.bmp', '*.tiff', '*.gif'],  # Image extensions to look for
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['swimmer', 'floater', 'boat', 'swimmer on boat', 'floater on boat',
                              'life jacket', 'ignored']
        ...
        self.frame_ids = {seq: [] for seq in self.seq_list}
        ...
        for seq in self.seq_list:
            if not self.data_is_zipped:
                ...
                for ext in self.config['IMG_EXTENSION']:
                    img_pattern = f"{self.gt_fol}/{seq}/img1/{ext}"
                    img_files = glob.glob(img_pattern)
                    for img_file in img_files:
                        frame_id = str(int(os.path.basename(img_file).split('.')[0]))
                        if frame_id:
                            self.frame_ids[seq].append(frame_id)
                self.frame_ids[seq] = sorted(self.frame_ids[seq], key=lambda x: int(x))
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        assert len(current_time_keys) == len(self.frame_ids[seq]), (f'Number of timesteps in seq {seq} '
                                                                    f'does not match the number of frames '
                                                                    f'in the image folder.'
                                                                    f' Number of timesteps: {len(current_time_keys)}, '
                                                                    f'number of frames: {len(self.frame_ids[seq])}.')
        time_keys_to_read_data_keys = dict(zip(current_time_keys, self.frame_ids[seq]))
        extra_time_keys = [x for x in read_data.keys() if x not in time_keys_to_read_data_keys.values()]
        ...
        for t in range(num_timesteps):
            time_key = str(t + 1)
            if time_keys_to_read_data_keys[time_key] in read_data.keys():
                try:
                    time_data = np.asarray(read_data[time_keys_to_read_data_keys[time_key]], dtype=np.float)
                except ValueError:
                    if is_gt:
                        raise TrackEvalException(
                ...
                if time_data.shape[1] >= 8:
                    ...
                    raw_data['classes'][t] = np.atleast_1d([1 for _ in range(time_data.shape[0])]).astype(int)  # class-agnostic
                else:
                    if not is_gt:
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            # which belonging to a distractor class.
