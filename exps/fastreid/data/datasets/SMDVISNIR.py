...
class SMDVISNIR(ImageDataset):

    _junk_pids = [0, -1]
    dataset_dir = 'ReID'
    dataset_url = ''
    dataset_name = "SMDVISNIR"
    ...
    def __init__(self, root='datasets', **kwargs):
        ...
        data_dir = osp.join(self.data_dir, 'SMDVISNIR-ReID')
        ...
        super(SMDVISNIR, self).__init__(train, query, gallery, **kwargs)

    def process_dir(self, dir_path, is_train=True):

        img_paths = glob.glob(osp.join(dir_path, '*.bmp'))
        pattern = re.compile(r'([-\d]+)_MVI_([-\d]+)_')
