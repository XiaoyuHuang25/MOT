...
def generate_trajectories(file_path: str, filter_visible: bool = True) -> np.ndarray:
    """
    Read MOT-style ground-truth/detection file and convert TLWH to TLBR.
...
def split_data(data, train_ratio: float):
    """
    Split a list of ids into train/test in fixed-size groups.
...
def get_gt_data(data_path: str, reid_img_dict: dict, id_offset: int):
    """
    Parse SMD MOT sequences under `data_path` and collect ReID samples.
    ...
    for seq in seqs:  # iterate over sequences
        ...
        gt = generate_trajectories(ground_truth_path, filter_visible=True)
        if gt.size == 0:
            continue
        ...
        for img_file in img_files:  # iterate over frames
                ...
                reid_img_dict[category_id][new_id].append(
                    {
                        ...
                        "bbox": [x1, y1, x2, y2],
                        "new_id": new_id,
                        "type": "train",  # temporary, overwritten later
                    }
                )

        id_offset += max_id_per_seq
        print(
            "Current id_offset", id_offset,
            "num_id_per_seq", max_id_per_seq,
            "num_labels_per_seq", num_labels_per_seq,
...
def generate_patches(
    ...
    for category_id in reid_img_dict:
        ...
        train_new_ids_list, test_new_ids_list = split_data(sorted_new_ids, train_ratio)
        ...
        if save_patches:
            ...
            for image_path in tqdm(reid_res, desc=f"Saving patches (class {category_id})"):
                ...
                for img_dict_tmp in reid_res[image_path]:
                    ...
                    if img_dict_tmp["type"] == "train":
                        ...
                        # For query images, shift camera id by an offset
                        query_camid_offset = 2000
                        seq_name = img_dict_tmp["seq_name"]
                        match = pattern.search(seq_name)
                        if match is None:
                            raise ValueError(f"Cannot parse camera id from seq_name: {seq_name}")
                        camid = int(match.group(2)) + query_camid_offset
                        separator = match.group(1)
                        seq_name = pattern.sub(separator + str(camid), seq_name, count=1)
            ...
            # Only check that expected files already exist
...
def make_parser():
    parser = argparse.ArgumentParser("Singapore Maritime Dataset ReID Builder")

    parser.add_argument(
        "--data_path",
        default="datasets/smd/images/",
        help="Path to SMD MOT-style data root",
    )
    parser.add_argument(
        "--save_path",
        default="datasets/ReID",
        help="Path to save the SMD-ReID dataset",
    )
    parser.add_argument(
        "--dataset_type",
        default="VIS",
        help="Dataset type, e.g. VIS, NIR, VISNIR",
    )
    parser.add_argument(
        "--train_ratio",
        type=float,
...
def main(args: argparse.Namespace):
    if not args.merge_train_val:
        save_path = os.path.join(args.save_path, f"SMD{args.dataset_type}-ReID")
    else:
        save_path = os.path.join(args.save_path, f"SMD{args.dataset_type}-TrainVal-ReID")
    ...
    if not args.merge_train_val:
        # 1) Train split: all trajectories go to train (no query/gallery)
        id_offset = 0
        reid_img_dict = {}
        ...
        # 2) Val split: split trajectories into query/gallery only
        id_offset = 0
        reid_img_dict = {}
        ...
        # Merged train_val split
        id_offset = 0
        reid_img_dict = {}
        print("Processing train_val data...")
