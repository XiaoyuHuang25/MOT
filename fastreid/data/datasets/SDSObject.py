...
class SDSObject(ImageDataset):

    _junk_pids = [0, -1]
    dataset_dir = 'ReID'
    dataset_url = ''
    dataset_name = "SDSObject"
    ...
    def __init__(self, root='datasets', **kwargs):
        ...
        data_dir = osp.join(self.data_dir, 'SDSObject-ReID')    # 'fast_reid/datasets/MOT17-ReID'
        ...
        super(SDSObject, self).__init__(train, query, gallery, **kwargs)

    def process_dir(self, dir_path, is_train=True):

        img_paths = glob.glob(osp.join(dir_path, '*.bmp'))
        pattern = re.compile(r'([-\d]+)_DJI_([-\d]+)_')
