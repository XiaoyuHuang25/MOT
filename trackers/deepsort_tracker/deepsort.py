        ...
        if self.asso == 'hmiou':
            matches_b, unmatched_tracks_b, unmatched_detections = \
                linear_assignment.min_cost_matching(
                    iou_matching.hmiou_cost, self.match_thresh, self.tracks,
...
class DeepSort(object):
    def __init__(self, 
        ...
        self.tracker = Tracker(
            metric, max_iou_distance=max_iou_distance, max_age=max_age, n_init=min_hits,
            match_thresh=match_thresh, asso=asso)

    def update(self, output_results, features):

        confidences = output_results[:, 4]
        bboxes = output_results[:, :4]
        bbox_xyxy = bboxes
        bbox_tlwh = self._xyxy_to_tlwh_array(bbox_xyxy)
        remain_inds = confidences > self.det_thresh
        bbox_tlwh = bbox_tlwh[remain_inds]
        confidences = confidences[remain_inds]

        detections = [Detection(bbox_tlwh[i], conf, features[i]) for i, conf in enumerate(
            confidences) if conf > self.det_thresh]
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([x1, y1, x2, y2, track_id, class_id + 1]))
        if len(outputs) > 0:
