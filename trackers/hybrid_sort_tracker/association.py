...
def associate_4_points(detections, trackers, iou_threshold, lt, rt, lb, rb, previous_obs, vdc_weight, iou_type=None):
    if (len(trackers) == 0):
...
def associate_4_points_with_score(detections, trackers, iou_threshold, lt, rt, lb, rb, previous_obs, vdc_weight,
                                  iou_type=None, TCM_first_step_weight=1.0):
    if (len(trackers) == 0):
        return np.empty((0, 2), dtype=int), np.arange(len(detections)), np.empty((0, 5), dtype=int)
...
def associate_4_points_with_score_with_reid(detections, trackers, iou_threshold, lt, rt, lb, rb, previous_obs, 
                                            ...
                                            longterm_reid_correction_thresh=0.0, TCM_first_step_weight=1.0):
    if (len(trackers) == 0):
        return np.empty((0, 2), dtype=int), np.arange(len(detections)), np.empty((0, 5), dtype=int)
        ...
        if emb_cost is None:
            ...
            if not with_longterm_reid:
                ...
                matched_indices = linear_assignment(weights[0] * (-(iou_matrix + angle_diff_cost)) +
                                                    weights[
                                                        1] * emb_cost + longterm_reid_weight * long_emb_dists)  # , thresh=thresh
...
def embedding_distance(tracks_feat, detections_feat, metric='cosine'):
    ...
    cost_matrix = np.maximum(0.0, cdist(tracks_feat, detections_feat,
                                        metric))  # Nomalized features, metric: cosine, [track_num, detection_num]
    return cost_matrix
